...
class LBAPIGSLB {
    ...
    public $globalnames;    //LBAPIGSLB_globalnames[]
    public $members;        //LBAPIGSLB_members[]          
    public $pools;          //LBAPIGSLB_pools[]
    public $topologies;     //LBAPIGSLB_topologies[]
        ...
        if($data != null){
            ...
            if(count($data->globalnames) > 0){
                foreach($data->globalnames as $object){
                    array_push($this->globalnames, new LBAPIGSLB_globalnames($object));
                }
            }

            if(count($data->members) > 0){
                foreach($data->members as $object){
                    array_push($this->members, new LBAPIGSLB_members($object));
                }
            }

            if(count($data->pools) > 0){
                foreach($data->pools as $object){
                    array_push($this->pools, new LBAPIGSLB_pools($object));
                }
            }

            if(count($data->topologies) > 0){
                foreach($data->topologies as $object){
                    array_push($this->topologies, new LBAPIGSLB_topologies($object));
                }
            }
            ...
            if(count($this->pools) > 0){
                foreach($this->pools as $object){
                    ...
                    foreach($this->members as $mobject){
                        $result = LBAPICLI::execute_raw("gslb", "--section pools --function edit --name " . $object->name . " --add_member " . $mobject->name);
                    }
                }
            }
    ...
    function add(){
        ...
        try {
            ...
            if( $this->name != null && $this->hostname != null ){
                ...
                $result = LBAPICLI::execute("gslb", $this);
                switch($result){
                    case 0: return false; break;
                    case 1: return true; break;
                    case 2: $this->function = "edit"; return LBAPICLI::execute("gslb", $this); break;
